...
export const navbarModule = {
  ...
  renderNavbar() {
    ...
    const easedSurface = utils.easeOutCubic(state.nav.surface.current);
  ...
  animate(now) {
    ...
    utils.stepAnimatedValue(state.nav.visible, springs.navVisible, delta);
    utils.stepAnimatedValue(state.nav.compact, springs.navCompact, delta);
    utils.stepAnimatedValue(state.nav.surface, springs.navSurface, delta);
    utils.stepAnimatedValue(state.nav.gestureStretch, springs.navGesture, delta);
    utils.stepAnimatedValue(state.hero.parallax, springs.heroParallax, delta);
    utils.stepAnimatedValue(state.cta.elasticY, springs.ctaElastic, delta);

    state.nav.visible.current = utils.clamp(state.nav.visible.current, 0, 1);
    state.nav.compact.current = utils.clamp(state.nav.compact.current, 0, 1);
    state.nav.surface.current = utils.clamp(state.nav.surface.current, 0, 1);
    state.nav.gestureStretch.current = utils.clamp(
