...
export const scrollSectionHintModule = {
  ...
  getBoundaryGapPx() {
    return utils.cssVar.remPx("--section-hint-boundary-gap", 4.8);
  },

  getNavbarBottom() {
    return this.navbar
      ? this.navbar.getBoundingClientRect().bottom
      : utils.cssVar.number("--nav-height", 78);
  },
  ...
  setHint(
    ...
    hintEl.style.opacity = `${
      utils.clamp(opacity, 0, 1) * utils.cssVar.number("--section-hint-visibility", 0.5)
    }`;
  ...
  refreshTimingVars() {
    this.hideDelayMs = utils.cssVar.timeMs("--section-hint-hide-delay", 1000);
    this.fadeDurationMs = utils.cssVar.timeMs("--section-hint-fade-duration", 500);
    this.showScrollDistancePx = utils.cssVar.lengthPx(
